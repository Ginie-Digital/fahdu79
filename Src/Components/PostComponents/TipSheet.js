...
const TipSheet = () => {
  ...
  const {show: homeBottomSheetVisibility, postId} = useSelector(
    state => state.hideShow.visibility.whoTippedSheet,
  );
  ...
  useEffect(() => {
    // Prevent running before ref is ready
    if (!bottomSheetRef.current) return;

    if (homeBottomSheetVisibility === -1) {
      // Close the sheet when visibility is -1
      bottomSheetRef.current.close();
    } else if (homeBottomSheetVisibility === 1) {
      // Open the sheet when visibility is 1
      handlePresentModalPress();
    }

    // Handle back button press only when sheet is visible
    const backHandler =
  ...
  const EachContent = ({item}) => {
    return (
      <Pressable
        style={styles.eachListContainer}
        ...
        <View style={{flexDirection: 'row', gap: responsiveWidth(2)}}>
          <View style={styles.imageContainer}>
            <Image
              source={{uri: item?.userDetail?.profile_image?.url}}
              resizeMethod="resize"
              resizeMode="cover"
            ...
            {item?.userDetails?.role === 'creator' ? (
              <View
                style={{
                  position: 'absolute',
                  transform: [
                    {translateX: responsiveWidth(8.4)},
                    {translateY: responsiveWidth(-5)},
                  ],
                }}>
                <DIcon
        ...
        <View style={styles.unblockWrapper}>
          ...
          />
          <Text style={[styles.name, {fontSize: responsiveFontSize(2.2)}]}>
            {item?.amount}
          </Text>
        </View>
      </Pressable>
      ...
      <View style={styles.contentContainer}>
        ...
        <BottomSheetFlatList
          ItemSeparatorComponent={() => (
            <LinearGradient
              colors={['#FFFDF650', '#43423D50', '#FFFDF650']}
              start={{x: 0, y: 0}}
              end={{x: 1, y: 0}}
          ...
          ListEmptyComponent={() => (
            <Text style={[styles.userName, {textAlign: 'center'}]}>
              Sorry! We did'nt recived any tips for this post
            </Text>
          )}
        />
      </View>
